...
test('selectMultipleChoice', () => {
    ...
    expect(game.round).toStrictEqual(round);
...
test('selectEstimate', () => {
    ...
    expect(game.round).toStrictEqual(estimateRound);
    ...
    expect(estimateRound.state).toBe(RoundState.SHOW_RESULTS);
    expect(round.question.alreadyAttempted(TeamColor.RED)).toBeTruthy();
    expect(round.question.completedBy).toContain(TeamColor.RED);
    expect(teamRed.points).toBe(200);
    expect(playerRedCamel.points).toBe(200);
    expect(game.round).toBe(estimateRound);
});